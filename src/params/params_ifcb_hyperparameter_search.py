...
class IFCBHyperparameterSearchParams(BaseModel):
    ...
    aspect_ratio_range: Optional[HyperparameterRange] = Field(
        None,
        description="Range/values for aspect_ratio parameter (camera frame aspect ratio)"
    )
    ...
    default_aspect_ratio: float = Field(1.36, description="Default aspect ratio if not searching")
